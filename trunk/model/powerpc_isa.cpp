...
{
    dbg_printf(" vpkuhum v%d, v%d, v%d\n\n", vrt, vra, vrb);
    ...
    for (i = 0; i < 4; i++) {
        ...
        for (j = 0; j < 2; j++) {
            ...
            bb = (uint8_t) (0x000000FF & (b.data[i] >> (8 * pos_in_data)));
...
{
    dbg_printf(" vpkuhus v%d, v%d, v%d\n\n", vrt, vra, vrb);
...
void ac_behavior( vmrghh ){
    ...
    for(int i = 0; i < 2; i++){
        t.data[2*i]   =  (a.data[i] & 0xffff0000) +  ( b.data[i] >> 16       ); 
...
void ac_behavior( vupkhpx ){
    ...
    for (int i=0; i < 2; i++){
        ...
        b2 = (hw1 >>  5) & 0x1f ; 
        b3 = (hw1 >>  0) & 0x1f ; 
        dbg_printf("hw1 = 0x%4x  hw1 & 0x8000 = 0x%4x\n", 
...
void ac_behavior( vupkhsb ){
    ...
    for (int i=0; i < 2; i++){
        ...
        dbg_printf("t[%d] = 0x%8x\t", 2*i, (int)t.data[2*i]); 
        dbg_printf("t[%d] = 0x%8x\n\n", 2*i+1 , (int)t.data[2*i+1]); 
    }
...
void ac_behavior( vupklpx ){
    dbg_printf(" vupklpx v%d, v%d\n\n", vrt, vrb);

    vec t;
    vec b = VR.read(vrb);
    for (int i=0; i < 2; i++){
        uint16_t hw0 = (b.data[i+2] & 0xffff0000) >> 1*16; 
        uint16_t hw1 = (b.data[i+2] & 0x0000ffff) >> 0*16; 
        uint8_t b0 = (hw0 & 0x8000) ?  0xff : 0; 
        uint8_t b1 = (hw0 >> 10) & 0x1f ; 
        uint8_t b2 = (hw0 >>  5) & 0x1f ; 
        uint8_t b3 = (hw0 >>  0) & 0x1f ; 
...
void ac_behavior( vupklsb ){
    ...
    dbg_printf(" vupklsb v%d, v%d\n\n", vrt, vrb);

    vec t;
    vec b = VR.read(vrb);
    for (int i=0; i < 2; i++){
        int8_t b0 = (b.data[i+2] & 0xff000000) >> 3*8; 
        int8_t b1 = (b.data[i+2] & 0x00ff0000) >> 2*8; 
        int8_t b2 = (b.data[i+2] & 0x0000ff00) >> 1*8; 
        int8_t b3 = (b.data[i+2] & 0x000000ff) >> 0*8; 
        dbg_printf(".b = {%#2x %#2x %#2x %#2x}\n", (unsigned char) b0, 
                                      (unsigned char) b1, 
                                      (unsigned char) b2,
                                      (unsigned char) b3); 
...
void ac_behavior( vupklsh ){
    ...
    dbg_printf(" vupklsh v%d, v%d\n\n", vrt, vrb);

    vec t;
    vec b = VR.read(vrb);
    for (int i=0; i < 2; i++){
        int16_t hw0 = (b.data[i+2] & 0xffff0000) >> 1*16; 
        int16_t hw1 = (b.data[i+2] & 0x0000ffff) >> 0*16; 
...
void ac_behavior( vspltisb ){
    ...
    dbg_printf(" vspltisb v%d, %d\n\n", vrt, sim);
    printf(" vspltisb v%d, %d\n\n", vrt,  sim);

    vec t(0);
    uint8_t value  = (uint8_t) sim;
    ...
    uint32_t t_i = (value << 24) + (value << 16) + (value << 8) + value; 

    printf("value = %#x = %d\n", (char)value, (signed int)value);
    printf("t_i = %#x\n\n", t_i); 
...
{
    dbg_printf(" vsum4sbs v%d, v%d, v%d\n\n", vrt, vra, vrb);
